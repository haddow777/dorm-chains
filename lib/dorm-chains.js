...
dorm_chain.save = function (handlers, err) {
	...
	if (typeof handlers === 'function') {
		...
		if (typeof handlers.preSave === 'function') {
			this.preSave(handlers.preSave, error_handler);
		}
...
dorm_chain.begin = function (handler) {
    this.__generic('begin', undefined, handler);
}

dorm_chain.commit = function (handler) {
    this.__generic('commit', undefined, handler);
}

dorm_chain.rollback = function (handler) {
    this.__generic('rollback', undefined, handler);
}

dorm_chain.savepoint = function (name, handler) {
    this.__generic('savepoint', name, handler);
}

dorm_chain.rollbackTo = function (name, handler) {
    this.__generic('rollbackTo', name, handler);
}

dorm_chain.runStoredProcedure = function (name, handler) {
    this.__generic('runStoredProcedure', name, handler);
}

dorm_chain.__generic = function (f_name, options, handler, err) {
	var self = this, vals = this.values;
	var error_handler;
	if (typeof promises[f_name] !== 'function') {
		throw new Error("DORM-CHAINS -> __generic: A valid DORM function name is required.");
	}
	if (typeof err === 'function'){
		error_handler = err;
	} 
		...
		error_handler = function(fail) {
			out(f_name + ' error handler');
			console.log(f_name + ' failed for ', fail);// + entity.type, entity.values, fail);
		}
	}
	error_handler = error_handler_creator(f_name, vals, err);
	console.log('Dorm-Chains -> __generic: Starting ', f_name);
	if (options === undefined) {
		options = this.promise;
	}
	if (this.promise === undefined) {
		this.promise = promises[f_name](options);
		this.promise = this.promise.then(function(win) { out(f_name); return win; }, error_handler);
	}
	else {
		this.promise = this.promise.then(function(win) { 
			//out('save'); console.log('PRESAVE WIN VALUE', util.inspect(win, true, 3, true)); 
			return promises[f_name](options); 
		}, error_handler);
	}
	if (typeof handler === 'function') {
		this.promise = this.promise.then(function(win) { 
			...
			handler(win, vals); 
			return 1; 
		}, function(fail) { console.log(f_name + ' custom handler failed: ', fail );});
	}
};
